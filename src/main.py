...
class MainGUI:
    def __init__(self, master):
        ...
        self.keras_ = 0  # default
        self.tensorflow_ = 0
        self.yolo_ = 1
        self.models_dir = ''  # gets updated as per user choice
        self.model_path = ''
        self.parent = master
        self.parent.title("OMNO ANNOTATION TOOL")
        ...
        self.canvas = Canvas(self.frame, width=700, height=700)
        self.canvas.grid(row=0, column=1, sticky=W + N)
        self.canvas.bind("<Button-1>", self.mouse_click)
        self.canvas.bind("<Motion>", self.mouse_move, "+")
        self.canvas.bind("<B1-Motion>", self.mouse_drag)
        self.canvas.bind("<ButtonRelease-1>", self.mouse_release)
        self.parent.bind(f"<Key-{PREVIOUS_IMAGE}>", self.open_previous)
        self.parent.bind(f"<Key-{NEXT_IMAGE}>", self.open_next)
        self.parent.bind(CANCEL_ANNOTATION, self.cancel_bbox)
        self.parent.bind(f"<Key-{ANNOTATE_MODE}>", self.set_class_label)
        self.parent.bind(f"<Control-{SAVE_KEY}>", self.save)
        ...
        self.objectListBox.bind("<<ListboxSelect>>", self.select_bbox)
        self.objectListBox.pack(fill=X, side=TOP)

        self.key_tracker = KeyTracker()
        self.parent.bind_all('<KeyPress>', self.key_tracker.report_key_press)
        self.parent.bind_all('<KeyRelease>', self.key_tracker.report_key_release)
        self.key_tracker.track('ALT_L')
        ...
        self.addThresh = Label(self.listPanel, text="Apparent Age").pack(fill=X, side=TOP)
        self.textBoxAge = Entry(self.listPanel, text="Enter Age value", state="disabled")
        self.textBoxAge.pack(fill=X, side=TOP)
        
        self.addThresh = Label(self.listPanel, text="Gender").pack(fill=X, side=TOP)
        self.is_male = BooleanVar()
        self.maleCheck = Checkbutton(self.listPanel, text="is male", onvalue = True, 
        ...
        self.frontalCheck = Checkbutton(self.listPanel, text="is frontal", onvalue = True, 
                      offvalue = False, variable=self.is_frontal, state="disabled")
        self.frontalCheck.pack(fill=X, side=TOP)

        self.addThresh = Label(self.listPanel, text="Visibility").pack(fill=X, side=TOP)
        self.face_visibility = IntVar()
        self.faceradio0 = Radiobutton(self.listPanel, text="0", variable=self.face_visibility, value=0, state="disabled")
        self.faceradio1 = Radiobutton(self.listPanel, text="1", variable=self.face_visibility, value=1, state="disabled")
        self.faceradio2 = Radiobutton(self.listPanel, text="2", variable=self.face_visibility, value=2, state="disabled")
        self.faceradio3 = Radiobutton(self.listPanel, text="3", variable=self.face_visibility, value=3, state="disabled")
        self.faceradio4 = Radiobutton(self.listPanel, text="4", variable=self.face_visibility, value=4, state="disabled")
        self.faceradio5 = Radiobutton(self.listPanel, text="5", variable=self.face_visibility, value=5, state="disabled")
        self.faceradio0.pack(fill=X, side=TOP)
        self.faceradio1.pack(fill=X, side=TOP)
        self.faceradio2.pack(fill=X, side=TOP)
        self.faceradio3.pack(fill=X, side=TOP)
        self.faceradio4.pack(fill=X, side=TOP)
        self.faceradio5.pack(fill=X, side=TOP)
        ...
        if self.keras_:
            ...
            self.body_face_association = {} # Dictionary with idx of body against associated face
            self.face_analytics = {}
    ...
    def set_class_label(self, index):
        labelidx = self.labelListBox.curselection()
        next_label = 0
        if len(labelidx) > 0:
            next_label = labelidx[0] + 1
            if next_label == self.labelListBox.size:
                next_label = 0

            self.labelListBox.selection_clear(labelidx[0])
        
        self.labelListBox.selection_set(next_label)
    ...
    def changeAnalytics(self, event=None):
        ...
        label_ = self.objectLabelList[idx]
        state_ = "disabled"
        if label_ == "face":
            state_ = "normal"

        for i in self.analytics_elements:
            i.config(state=state_)
    ...
    def select_bbox(self, event):
        ...
        if type(self.current_selection) != type(None):
            self.canvas.itemconfigure(self.bboxIdList[self.current_selection], width=2)

            if self.key_tracker.pressed:
                # Here we make the association
                prev_label = self.objectLabelList[self.current_selection]
                new_label = self.objectLabelList[idx]

                if new_label != prev_label:
                    if new_label == "body":
                        self.body_face_association[idx] = self.current_selection
                    else:
                        self.body_face_association[self.current_selection] = idx

                    self.canvas.itemconfigure(self.bboxIdList[idx], outline="white")
                    self.canvas.itemconfigure(self.bboxIdList[self.current_selection], outline="white")
                    self.objectListBox.itemconfigure(idx, bg="black")
                    self.objectListBox.itemconfigure(self.current_selection, bg="black")
        
        # self.canvas.delete(self.bboxIdList[idx])
        self.canvas.itemconfigure(self.bboxIdList[idx], width=5)
    ...
    def add_label_(self, text):
        curr_label_list = self.labelListBox.get(0, END)
        curr_label_list = list(curr_label_list)
        if text not in curr_label_list:
            self.labelListBox.insert(END, text)
    ...
    def automate(self):
        ...
        if self.tensorflow_ :
            ...
            det = detect.detect_single(img=open_cv_image, model=self.yolo_model)[0]
            # import pdb; pdb.set_trace()
            boxes = [det[:, :4].numpy()]
            labels = [[int(i.item()) for i in det[:, -1]]]
            scores = [det[:, -2].numpy()]
            config_labels = config.labels_to_names_yolo
            m_name = "yolo_body_face"
